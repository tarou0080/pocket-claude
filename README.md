...
Proxied models can also opt into a **trimmed tool set**, off by default. If you set `proxyDisallowedTools` (an array of tool names) in `config.json`, sessions on a proxied model add that list to `--disallowed-tools` on top of the always-disabled `AskUserQuestion`. A recommended 23-tool list (keeping `Bash`, `Edit`, `Read`, `Write` and `WebFetch`) ships in `config.example.json` for you to copy. This only helps endpoints where prompt caching does not apply — e.g. a small local model behind a translation proxy, where the full ~28 tool definitions are re-sent and re-processed on every turn (roughly 70% of the input; one local-Ollama measurement went from ~100,000 to ~18,500 characters per request, with the first response landing in ~13 seconds instead of ~40 and usable context growing from ~50K to ~70K tokens). If your `proxyModels` entry actually reaches a real Claude behind a corporate gateway, prompt caching still applies there, so leave `proxyDisallowedTools` unset — trimming would only cost you tools for no benefit. Models talking to Anthropic directly are never trimmed regardless of this setting.
