...
- **Upgrading to v2.12.0 converts your existing data once, after taking a backup.** The first start unifies each conversation's two IDs into one (see the changelog). Before changing anything it writes `migration-backup-<timestamp>.tar.gz` into the install directory, holding `sessions/`, `logs/`, `schedules.json` and `scheduled-posts.json` - this is the only way back, so keep it until you are satisfied, then delete it. Restore by extracting it over the install directory. That one-time conversion also deleted live logs that were redundant with, or unreachable from, the CLI's own transcripts, which could free a substantial amount of disk (this happened only during the v2.12.0 conversion; from v2.12.3 onward live logs are kept under the retention window described in Retention below). The conversion runs only once and later versions do not take this backup. If the server fails to start, set `PC_SKIP_STARTUP_MIGRATION=1` to boot without converting.
...
- **Retention**: `logs/*.jsonl` (live logs) and `sessions/*.json` (session metadata) are swept on the same age basis, run at startup and once a day thereafter - not tied to CLI process lifetime or whether the CLI's own transcript exists. The age limit follows Claude Code's own `cleanupPeriodDays` setting (checked in `managed-settings.json` first, then user `settings.json`; project/local settings are not read; defaults to 30 days if unset or invalid). The Claude Code config directory itself follows `CLAUDE_CONFIG_DIR` if set, otherwise `~/.claude`.
