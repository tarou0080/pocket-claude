...
Proxied models also get a **trimmed tool set**. Prompt caching does not apply on these endpoints, so the full tool definitions (~28 tools) are re-sent and re-processed on every turn — roughly 70% of the input. For `proxyModels` sessions only, pocket-claude keeps `Bash`, `Edit`, `Read`, `Write` and `WebFetch` and disables the rest, which cuts a request from ~100,000 to ~18,500 characters. The point is latency and context headroom, not cost. Override the disabled list with `proxyDisallowedTools` (an array of tool names) in `config.json`; models talking to Anthropic directly are never trimmed.
