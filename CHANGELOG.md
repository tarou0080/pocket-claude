...
### Added
- **Proxied models can now opt into a trimmed tool set** - Some endpoints reached through `proxyModels` (a small local Ollama model behind a translation proxy, for example) don't benefit from prompt caching, so the full set of ~28 tool definitions is re-sent and re-processed on every single turn - about 70% of the whole input. If you set `proxyDisallowedTools` in `config.json`, sessions on a proxied model add that list to `--disallowed-tools`; a recommended 23-tool list (keeping `Bash`, `Edit`, `Read`, `Write` and `WebFetch`) ships in `config.example.json` to copy from. One local-Ollama measurement went from ~100,000 to ~18,500 characters per request, first response in ~13 seconds instead of ~40, and ~70K tokens of usable context instead of ~50K. This is opt-in and off by default: some `proxyModels` entries reach a real Claude behind a corporate gateway, where prompt caching still applies and trimming would only cost tools for no benefit. Models that talk to Anthropic directly are never trimmed.
