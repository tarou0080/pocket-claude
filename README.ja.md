...
プロキシ経由のモデルでは、あわせて**ツール定義を絞ることもできます（既定OFF・オプトイン）**。`config.json` に `proxyDisallowedTools`（ツール名の配列）を設定すると、プロキシ経由モデルのセッションでは常に無効化される `AskUserQuestion` に加えてそのリストが `--disallowed-tools` へ渡されます。`Bash`・`Edit`・`Read`・`Write`・`WebFetch` を残す推奨23ツールのリストを `config.example.json` に用意してあるので、そこからコピーして使ってください。これが効くのはプロンプトキャッシュが効かない経路（翻訳プロキシ配下の小型ローカルモデルなど）だけです。ツール定義28個を毎ターン丸ごと再送・再処理するため入力の約7割をツール定義が占めており、ローカルOllamaでの実測例では1リクエストが約100,000文字→約18,500文字、初回応答が約40秒→約13秒、会話に使えるコンテキストが約5万→約7万トークンになりました。`proxyModels` の接続先が社内ゲートウェイ経由の本物のClaudeであるような場合はプロンプトキャッシュが効くため、`proxyDisallowedTools` は設定しないでください（削っても得がなく、使えるツールが減るだけです）。Anthropic へ直接つなぐモデルはこの設定に関わらず対象外です。
