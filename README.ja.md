...
セッションが使えるツールは**ブラウザの設定画面**（折りたたみの「上級者向け: ツール構成」セクション）から制限できます。「Claude直」と「プロキシ経由」の2系統が独立していて、どちらも allowlist 方式です——未設定なら何も絞られません（全ツール利用可）。ツール一覧はハードコードではなく、セッション起動時にCLIが送ってくる `system/init` イベントから自動学習して `tools-catalog.json` に保存するため、Anthropicがツールを追加しても追従します。変更は**次に起動するセッションから反映**され（実行中の会話はそのまま）、`AskUserQuestion` はこの設定に関わらず常に無効です（ヘッドレスモードでは対話的に解決できないため）。
...
- **パーミッションモード** - より安全な操作のため `permissionMode: "acceptEdits"` を使用。値はそのまま `claude --permission-mode` に渡されるため、指定できる値はインストール済みCLIが受け付けるもの（`claude --help` を参照）。pocket-claude側では一覧を持たない。未知の値を書くとCLIが起動を拒否する
