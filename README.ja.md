...
プロキシ経由のモデルでは、あわせて**ツール定義を絞ります**。これらのエンドポイントではプロンプトキャッシュが効かず、ツール定義28個を毎ターン丸ごと再送・再処理するため、入力の約7割をツール定義が占めます。`proxyModels` のセッションに限り `Bash`・`Edit`・`Read`・`Write`・`WebFetch` の5つだけを残して他を無効化し、1リクエストを約100,000文字→約18,500文字に圧縮します。狙いはコストではなく、応答速度と会話に使えるコンテキストの余裕です。無効化するツールは `config.json` の `proxyDisallowedTools`（ツール名の配列）で上書きできます。Anthropic へ直接つなぐモデルは対象外です。
